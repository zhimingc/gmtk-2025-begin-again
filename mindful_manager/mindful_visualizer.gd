...
func update_visualization(progress : float) -> void:
	...
	if disturb_timer_circle_fade and timer_circle.modulate == Color.WHITE and progress <= 0.5:
		...
		disturb_fade_tween.tween_property(timer_circle, "modulate", Color.TRANSPARENT, 0.25)
...
func reset_visualisations() -> void:
...
func trigger_blackout() -> void:
	...
	blackout_tween.tween_property(blackout_obj, "modulate", Color.BLACK, 1.0)
